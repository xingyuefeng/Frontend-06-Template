  ...
  send(connection) {
    return new Promise((resolve, reject) => {
      ...
      connection.on('data', (data) => {
        ...
        if (parser.bodyParser && parser.bodyParser.isFinished) {
          resolve(parser.bodyParser.content.join(''))
          connection.end()
        }
      })
...
class ResponseParser {
  constructor() {
    ...
    this.WATIING_STATUS_LINE = 0  // /r
    this.WATIING_STATUS_LINE_END = 1 // /n
  ...
  receiveChar(char) {
    if (this.current === this.WATIING_STATUS_LINE) {
      ...
      if (char === ':') {
        ...
        // body 跟header相关 在header结束的时候转化body
