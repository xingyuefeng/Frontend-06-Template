...
function layout(element) {
  ...
  if (style.flexWrap === 'nowrap' || isAutoMainSize) {
    flexLine.corssSpace =
      style[corssSize] !== undefined ? style[corssSize] : corssSpace
  } else {
    flexLine.corssSpace = corssSpace
  }
  if (mainSpace < 0) {
  ...
  flexLines.forEach((items) => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (align === 'flex-start') {
        ...
        itemStyle[corssEnd] =
          itemStyle[corssStart] + corssSign * itemStyle[corssSize]
      }
      if (align === 'flex-end') {
        itemStyle[corssStart] =
          itemStyle[corssEnd] - corssSign * itemStyle[corssSize]
        itemStyle[corssEnd] = corssBase + corssSign * lineCorssSize
      }
      if (align === 'center') {
        itemStyle[corssStart] =
          corssBase + (corssSign * (lineCorssSize - itemStyle[corssSize])) / 2
        itemStyle[corssEnd] =
          itemStyle[corssStart] + corssSign * itemStyle[corssSize]
      }
      if (align === 'stretch') {
        ...
        itemStyle[corssSize] =
          corssSign * (itemStyle[corssEnd] - itemStyle[corssStart])
      }
    }
